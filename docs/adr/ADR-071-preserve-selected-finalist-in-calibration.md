...
2. **Store a selected trial index on every experiment.** Exact, but unnecessary schema growth: the
   ...
   deterministic. **ADR-079 follow-up:** this rejection applies only to in-memory calibration.
   Persisted pool reporting does not retain `select_by`, and strategy name is ambiguous after
   refinement, so ADR-079 adds the index to new real experiments.
...
- ADR-079 applies the same identity guarantee to persisted real-pool reporting.
