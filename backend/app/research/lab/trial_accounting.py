"""Whole-search multiple-testing accounting (ADR-046)."""
...
def whole_search_deflated_sharpes(
    ...
    Notes:
        ...
        it estimates the trial dispersion. ``lifetime_trials`` supplies the cumulative selection
        breadth. Historical candidate distributions were not persisted, so combining them here
        would require invented data; ADR-046 records the current-run dispersion approximation.
    """
    ...
    sr_std = max(float(np.std(candidate_sharpes, ddof=1)), 1e-6)
