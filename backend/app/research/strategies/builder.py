...
def build_strategy(config: StrategyConfig) -> BaseStrategy:
    ...
    if isinstance(config, VolManagedMomentumConfig):
    ...
    if isinstance(config, DonchianATRTrailConfig):
        return DonchianATRTrailStrategy(
    ...
    if isinstance(config, RegimeFilteredTrendConfig):
        return RegimeFilteredTrendStrategy(
            fast=config.fast,
            slow=config.slow,
