...
STRATEGY_CATALOG: list[StrategySchema] = [
    ...
    StrategySchema(
        name="two_timescale_reversion",
        label="Two-Timescale Residual Reversion",
        category="Mean Reversion",
        summary="Tracks a slowly drifting fair value and trades the fast wobble around it, "
        "fading price back toward the level.",
        description=(
            "Estimates the level with an exponentially weighted mean over `level_span`, then "
            "z-scores the residual (close - level) against its OWN rolling standard deviation "
            "over a separate, shorter `scale_window`; position is -clip(z / k, -1, 1). Every "
            "other reverting strategy here uses one window for both jobs, and one window cannot "
            "do both -- short enough to track a drifting level absorbs the deviation into it, "
            "long enough to isolate the deviation carries the level's lag into it. Trailing "
            "statistics only -- no look-ahead."
        ),
        citations=[
        ...
        parameters=[
            ParamSchema(
                ...
                maximum=200,
                label="Level span",
                description="EWM span estimating the slowly drifting level the price reverts to",
            ),
            ParamSchema(
                name="scale_window",
                type="int",
                ...
                minimum=3,
                maximum=60,
                label="Deviation scale window",
                description="Separate, shorter window scaling the residual -- independent of the level span",
            ),
            ParamSchema(
                name="k",
                type="float",
                ...
                maximum=3.0,
                step=0.25,
                label="Z-score divisor",
                description="Residual z-score at which the position reaches full size",
            ),
        ],
    ),
]
